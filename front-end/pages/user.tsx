...
export async function getServerSideProps(context: undefined) {
  ...
  if (session) {
    ...
    const url = `http://127.0.0.1:3080/users/find/${username}`;
    const userData = await getApiWithAxios(url, session.token);
    //const userData = await useApiGetWithBearer(url, session.token);
